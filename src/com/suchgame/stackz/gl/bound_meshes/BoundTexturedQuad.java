...
public class BoundTexturedQuad extends ABoundMesh {
	...
	public int render(RenderContext rc, FloatBuffer vb, ShortBuffer frameIndexBuffer){
		...
		if (positionDirty){
			TexturedQuad.position(vb, vbIndex, position);
			positionDirty = false;
		}
	...
	public boolean contains(float[] xy) {
		return position.containsXY(xy[0], xy[1]);
	}
	...
	public void putReversedIndices(ShortBuffer quadsIndices) {
		TexturedQuad.writeReversedIndices(quadsIndices, indexBuffer);
	}
}
