...
public class AnimatedBoundTexturedQuad extends BoundTexturedQuad {
	...
	public void positionY(float top) {
		startPosition.set(getCurrentPosition());
		targetPosition.setY(top, top - startPosition.height());
		posTransition.setDirect(0);
		posTransition.set(1);
		positionDirty = true;
	}
	...
	public int render(RenderContext rc, FloatBuffer vb, ShortBuffer frameIndexBuffer){
		...
		if (positionDirty){
            TexturedQuad.position(vb, vbIndex, getPosition(rc.frameNanoTime));
			positionDirty = !posTransition.isDone(rc.frameNanoTime);
		}
	...
	public long getPositionAnimationDuration() {
		return posTransition.getDuration();
	}
}
