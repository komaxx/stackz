...
public class AnimatedBoundSharedTexturedQuad extends ABoundMesh {
	protected static FloatBuffer vertexBuffer = RenderConfig.VBO_RENDERING ? TexturedQuad.allocateQuads(1) : null;
	...
	public int render(RenderContext rc, FloatBuffer vb, ShortBuffer frameIndexBuffer){
		...
		if (dirty){
			int firstVertexFloatIndex = firstVertexIndex * TexturedVertex.STRIDE_FLOATS;
			
            TexturedQuad.position(vb, firstVertexFloatIndex, getPosition(rc.frameNanoTime));
			
   			float nowAlpha = alpha.get(rc.frameNanoTime);
   			TexturedQuad.setAlpha(vb, firstVertexFloatIndex, nowAlpha, nowAlpha, nowAlpha, nowAlpha);
   			dirty = !alpha.isDone(rc.frameNanoTime);
    		
    		if (alpha.getLast() < 0.05f) return 0;

			if (rotateTexCoords)
				TexturedQuad.setUVMappingRotated(vb, firstVertexFloatIndex, texCoordsUv);
			else
				TexturedQuad.setUVMapping(vb, firstVertexFloatIndex, texCoordsUv);
