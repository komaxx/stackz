...
public class BoundTexturedGrid extends ABoundMesh {
	...
	public BoundTexturedGrid(int vCols, int vRows){
		...
		vertexBuffer = RenderConfig.VBO_RENDERING ? Vertex.allocateColorVertices(vRows * vCols) : null;
	...
	public int render(RenderContext rc, FloatBuffer vb, ShortBuffer frameIndexBuffer){
		...
		if (positionDirty){
			applyPosition(vb, vbIndex);
			positionDirty = false;
		}
		
		if (uvBoundsDirty){
			applyUvBounds(vb, vbIndex);
			uvBoundsDirty = false;
		}
	...
	private void applyUvBounds(FloatBuffer vb, int offset) {
	...
	private void applyPosition(FloatBuffer vb, int offset) {
		// precompute x-Values
