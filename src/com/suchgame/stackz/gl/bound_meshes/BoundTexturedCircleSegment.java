...
import com.suchgame.stackz.gl.primitives.TexturedCircleSegment;
...
public class BoundTexturedCircleSegment extends ABoundMesh {
	...
	public BoundTexturedCircleSegment(int segments){
		...
		vertexBuffer = RenderConfig.VBO_RENDERING ? TexturedCircleSegment.allocate(segments) : null;
	...
	public int render(RenderContext rc, FloatBuffer vb, ShortBuffer frameIndexBuffer){
		...
		if (positionDirty){
			TexturedCircleSegment.position(vb, firstVertexIndex, 
					centerPosition[0], centerPosition[1], centerPosition[2],
					radius, startAngle, endAngle, segments);
			positionDirty = false;
		}
		...
		if (alphaDirty){
			TexturedCircleSegment.setAlpha(vb, vbIndex, alpha, alpha, segments);
			alphaDirty = false;
		}
		
		if (texCoordsDirty){
			TexturedCircleSegment.setUVMapping(vb, vbIndex, 
					hubTexCoordsUv.x, hubTexCoordsUv.y,
					rimTexCoordsUv.x, rimTexCoordsUv.y, segments);
