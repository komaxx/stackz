...
public class AtlasPainter {
	public static enum FillOrientation{
		/**
		 * The AtlasPainter will choose the orientation that takes up less space
		 */
		AUTO_SELECT, 
		HORIZNOTAL, 
		VERTICAL
	...
	public static Rect[] drawAtlas(Resources r, int[] drawableIds, Texture t, int padding, FillOrientation orientation){
		return drawAtlas(r, drawableIds, t, new Rect(0,0,t.getWidth(), t.getHeight()), padding, orientation);
	}
	
	public static Rect[] drawAtlas(Resources r, int[] drawableIds, Texture t, int padding){
		return drawAtlas(r, drawableIds, t, new Rect(0,0,t.getWidth(), t.getHeight()), padding, FillOrientation.AUTO_SELECT);
	}
	 ...
	 * </br>
	 * Make sure that the texture is currently bound!
	 * </br>
	 * <b>GLThread only. GL thread *must* have been created before.</b>
	 */
	public static Rect[] drawAtlas(
			Resources r, int[] drawableIds, Texture t, 
			Rect paintBounds, int padding, FillOrientation orientation){
		...
		Rect horizontalBoundingRect = new Rect(paintBounds);
		if (orientation != FillOrientation.VERTICAL){
			positionHorizontally(ret, paintBounds.width(), padding);
			horizontalBoundingRect = getBoundingRect(ret);
		}
		
		Rect boundingRect = new Rect(paintBounds);
		if (orientation != FillOrientation.HORIZNOTAL){
			positionVertically(ret, paintBounds.width(), padding);
			boundingRect = getBoundingRect(ret);
		}
