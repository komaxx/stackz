...
public class InteractionTestNode extends Node{
	...
	protected boolean onInteraction(InteractionContext interactionContext) {
		...
		if (firstPointer.isActive()){
			...
			ColorQuad.positionXY(quadsData, 10 * ColorQuad.QUAD_FLOATS, testRect);
			...
			ColorQuad.position(quadsData, 13 * ColorQuad.QUAD_FLOATS, 
			...
			if (testRect.contains(interactionX, interactionY)){
				ColorQuad.color(quadsData, 10 * ColorQuad.QUAD_FLOATS, 1, 1, 0, 1);
			} else {
				ColorQuad.color(quadsData, 10 * ColorQuad.QUAD_FLOATS, 0.2f, 0.2f, 0.6f, 1);
			}
		}
...
//		float centerDistanceY = renderContext.surfaceHeight/4;
		...
		ColorQuad.position(quadsData, 4 * ColorQuad.QUAD_FLOATS,
				-10000, centerDistance + size/2, z, 10000, centerDistance-size/2, z);
		ColorQuad.color(quadsData, 4 * ColorQuad.QUAD_FLOATS, 0.4f, 0, 0, 1);
		
		ColorQuad.position(quadsData, 5 * ColorQuad.QUAD_FLOATS, 
				-10000, -centerDistance + size/2, z, 10000, -centerDistance-size/2, z);
		ColorQuad.color(quadsData, 5 * ColorQuad.QUAD_FLOATS, 0, 0.4f, 0, 1);
		
		ColorQuad.position(quadsData, 6 * ColorQuad.QUAD_FLOATS, 
				-centerDistance - size/2, 10000, z, -centerDistance + size/2, -10000, z);
		ColorQuad.color(quadsData, 6 * ColorQuad.QUAD_FLOATS, 0, 0, 0.4f, 1);
		
		ColorQuad.position(quadsData, 7 * ColorQuad.QUAD_FLOATS, 
				centerDistance - size/2, 10000, z, centerDistance + size/2, -10000, z);
		ColorQuad.color(quadsData, 7 * ColorQuad.QUAD_FLOATS, 0, 0.4f, 0.4f, 1);
		...
		for (int i = 8; i < 8+INTERACTION_QUADS; i++){
			...
			ColorQuad.position(quadsData, i * ColorQuad.QUAD_FLOATS, -20,20,(i-12)*-100,20,-20,(i-12)*-100);
...
//				quadsData.get(13 * ColorQuad.QUAD_FLOATS),
//				quadsData.get(13 * ColorQuad.QUAD_FLOATS + 1),
//				0);
