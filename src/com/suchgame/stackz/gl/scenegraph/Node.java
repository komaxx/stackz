...
public class Node {
	...
	/**
	 * The node containing this one in the SceneGraph
	 */
	private Node parent;
	...
	/**
	 * Only relevant when clustering. This will be used as sorting index of all nodes
	 * inside of a clustering. The logic follows the ordinary zLevel: Bigger value ==
	 * 'deeper' in the scene.
	 */
	public int clusterZLevel = 0;
	...
	/**
	 * The unique id used to associate textures or TextureSegments with a node.
	 */
	protected final int textureOwnerId = TextureStore.createOwnerID(); 
	...
	/**
	 * When false, this node and the lower branch will not react to interaction events,
	 * onInteraction will not be called. This is for runtime enabling/disabling. 
	 * If handlesInteraction is set to <code>false</code>, this flag is not relevant.
	 */
	...
	/**
	 * defines the target function when depth_testing. Only active when depth testing
	 * is activated. Default: GLES20.GL_LESS. Useful alternative: GL_ALWAYS to always
	 * pass the depth test but writing the depth value anyway
	 */
	public int depthFunction = GLES20.GL_LESS;
	
	/**
	 * glRunnables are executed when the node is processed in the rendering thread.
	 */
	protected final ArrayList<IGlRunnable> glRunnables = new ArrayList<IGlRunnable>();
	protected final ArrayList<IGlRunnable> glRunnablesCopy = new ArrayList<IGlRunnable>();		// used during rendering.
	 ...
	 * <b>NOTE</b> Do not just set to <code>true</code> but always to RenderConfig.VBO_RENDERING!
	 * Otherwise, vbo-rendering may be attempted and result in an error when not supported
	 * on the local device. 
	 */
	public boolean useVboPainting = RenderConfig.VBO_RENDERING;
	...
	/**
	 * Only of interest when the node does transformations AND interaction.
	 * If yes, the node did transformations in onTransform AND transformations
	 * int onTransformInteractionContext.
	 */
	private boolean interactionContextTransformed = false;
	...
	// //////////////////////////////////////////////////////////////////
	// these are the actual render/interaction functions when rendering an optimized linearization
	...
	public void renderSelf(RenderContext rc){
		...
		if (parentVisible && visible){
			// re-apply state since runnables could have changed the state!
			applyStateChangeRendering(rc);
			onRender(rc);
		}
		
		if (RenderConfig.GL_DEBUG) RenderUtil.checkGlError("after render in node: "+ this);
	}
	...
	public final boolean handleInteraction(InteractionContext interactionContext) {
		return 
			...
			interactionContext.interactiveStack.peek().b &&
	...
	public void transformInteraction(InteractionContext interactionContext) {
		interactionContext.interactiveStack.push().b &= interactive;
		
		if (scissorTest != DONT_CARE){
			interactionContext.activateScissorTest(scissorTest==ACTIVATE);
		}
		if (visible && contextTransformed && interactive && interactionContext.interactiveStack.peek().b){
			onTransformInteraction(interactionContext);
			interactionContextTransformed = true;
		} else {
			interactionContextTransformed = false;
		}
	}
	
	public void untransformInteraction(InteractionContext interactionContext) {
		interactionContext.interactiveStack.pop();
		if (interactionContextTransformed) onUnTransformInteraction(interactionContext);
	}
	...
	private final void processGlRunnables(RenderContext rc) {
		...
		synchronized (glRunnables) {
			...
			for (int i = 0; i < l; i++){
				glRunnablesCopy.add(glRunnables.remove(i));
				i--;
				l--;
			}
		}

		for (IGlRunnable r : glRunnablesCopy) r.run(rc);
		glRunnablesCopy.clear();
	}
	...
	protected void applyStateChangeRendering(RenderContext renderContext) {
		...
		else if (draws) renderContext.bindVBO(vbo.getHandle());
		...
		if (depthTest != DONT_CARE){
			boolean nowActive = depthTest==ACTIVATE;
			renderContext.activateDepthTest(nowActive);
			if (nowActive){
		...
		if (renderProgramIndex != -1){
	    	renderContext.applyMvpMatrixToShader();
		}
	}
	...
	private void recreateVbo() {
		if (vbo != null && RenderConfig.VBO_RENDERING) vbo.create();
	}
	 ...
	 * to their z value to ensure correct transparent drawings. The positive z-axis points
