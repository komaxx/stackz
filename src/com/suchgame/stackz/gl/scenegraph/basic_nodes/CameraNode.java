...
import com.suchgame.stackz.gl.scenegraph.interaction.InteractionEvent;
import com.suchgame.stackz.gl.scenegraph.interaction.MotionEventHistory;
...
public class CameraNode extends Node implements ICameraInfoProvider {
	...
	public boolean onTransform(SceneGraphContext sgContext) {
		...
		MatrixUtil.setIdentityM(stackModelMatrix);
...
//		return ret;
	...
	private void computeZoomSnap(InteractionContext interactionContext) {
		...
		int nuIndex = 0;
		float bestDistance = (float) Math.sqrt( Math.abs(eyePoint[2]-ZOOM_SNAP_LEVELS[0]) );
		for (int i = 1; i < ZOOM_SNAP_LEVELS.length; i++){
			float distance = (float) Math.sqrt( Math.abs(eyePoint[2]-ZOOM_SNAP_LEVELS[i]) );
