package com.suchgame.stackz.gl.texturing;
...
public class Texture {
	 ...
	 * Not 'inlayed' by default.
	 */
	public RectF getUvCoords(Rect r) {
		return getUvCoords(new RectF(), r, false);
	}
	
	/**
	 * Translates pixel coords into uvCoords. Creates and returns a new RectF.
	 */
	public RectF getUvCoords(Rect r, boolean inlayed) {
		return getUvCoords(new RectF(), r, inlayed);
	}
	
	/**
	 * Translates pixel coords into uv coords in the texture. Returns a new RectF
	 * with the uv coords set, not inlayed!
	 */
	public RectF getUvCoords(float left, float top, float right, float bottom) {
		return getUvCoords(left, top, right, bottom, false);
	}
	
	public RectF getUvCoords(float left, float top, float right, float bottom, boolean inlayed) {
		RectF ret = new RectF(
		...
		if (inlayed){
			float halfPixelOffset = 0.5f / width;
			ret.inset(halfPixelOffset, halfPixelOffset);
		}
