package com.suchgame.stackz.gl.primitives;
...
public class ColorQuad {
	...
	public static final int QUAD_FLOATS = VERTEX_COUNT * Vertex.COLOR_VERTEX_DATA_STRIDE_FLOATS;
	public static final int QUAD_BYTES = VERTEX_COUNT * Vertex.COLOR_VERTEX_DATA_STRIDE_BYTES;

	private static final int UPPER_LEFT_XYZ_OFFSET = 0;
	private static final int LOWER_LEFT_XYZ_OFFSET = Vertex.COLOR_VERTEX_DATA_STRIDE_FLOATS;
	private static final int LOWER_RIGHT_XYZ_OFFSET = 2 * Vertex.COLOR_VERTEX_DATA_STRIDE_FLOATS;
	private static final int UPPER_RIGHT_XYZ_OFFSET = 3 * Vertex.COLOR_VERTEX_DATA_STRIDE_FLOATS;

	private static final int UPPER_LEFT_COLOR_OFFSET = Vertex.COLOR_VERTEX_DATA_COLOR_OFFSET;
	private static final int LOWER_LEFT_COLOR_OFFSET = Vertex.COLOR_VERTEX_DATA_COLOR_OFFSET + Vertex.COLOR_VERTEX_DATA_STRIDE_FLOATS;
	private static final int LOWER_RIGHT_COLOR_OFFSET = Vertex.COLOR_VERTEX_DATA_COLOR_OFFSET + 2 * Vertex.COLOR_VERTEX_DATA_STRIDE_FLOATS;
	private static final int UPPER_RIGHT_COLOR_OFFSET = Vertex.COLOR_VERTEX_DATA_COLOR_OFFSET + 3 * Vertex.COLOR_VERTEX_DATA_STRIDE_FLOATS;
	...
	public static void color(FloatBuffer data, int offset, int vertex, float[] colorRGBA) {
		...
		case UPPER_LEFT_VERTEX:
			offset += UPPER_LEFT_COLOR_OFFSET;
			break;
		case UPPER_RIGHT_VERTEX:
			offset += UPPER_RIGHT_COLOR_OFFSET;
			break;
		case LOWER_LEFT_VERTEX:
			offset += LOWER_LEFT_COLOR_OFFSET;
			break;
		case LOWER_RIGHT_VERTEX:
			offset += LOWER_RIGHT_COLOR_OFFSET;
			break;
		}
	...
	public static final void color(FloatBuffer data, int offset, float r, float g, float b, float a){
		// upper left vertex
		data.position(offset + UPPER_LEFT_COLOR_OFFSET);
		data.put(r);
		data.put(g);
		data.put(b);
		data.put(a);
		...
		// lower left vertex
		data.position(offset + LOWER_LEFT_COLOR_OFFSET);
		data.put(r);
		data.put(g);
		data.put(b);
		data.put(a);
		...
		// lower right vertex
		data.position(offset + LOWER_RIGHT_COLOR_OFFSET);
		data.put(r);
		data.put(g);
		data.put(b);
		data.put(a);
		...
		// upper right vertex
		data.position(offset + UPPER_RIGHT_COLOR_OFFSET);
		data.put(r);
		data.put(g);
		data.put(b);
		data.put(a);
	...
	public static void positionY(FloatBuffer data, int offset, float top, float bottom) {
		data.put(offset + UPPER_LEFT_XYZ_OFFSET + 1, top);
		data.put(offset + UPPER_RIGHT_XYZ_OFFSET + 1, top);
		data.put(offset + LOWER_LEFT_XYZ_OFFSET + 1, bottom);
		data.put(offset + LOWER_RIGHT_XYZ_OFFSET + 1, bottom);
	}
