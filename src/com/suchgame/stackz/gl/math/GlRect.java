...
public class GlRect {
	...
	public boolean contains(GlRect r) {
	...
	/**
	 * Simply multiplies each edge with the given scalar.
	 */
	 ...
	 * inside, does nothing.
	 */
	public void enlargeToContain(float x, float y) {
		if (x < left) left = x;
	...
	/**
	 * Enlarges / shrinks the rect by the given scale factors while
	 * retaining the center point.
	 */
	public void enlargeByFactor(float xFactor, float yFactor){
		float centerX = centerX();
		float centerY = centerY();
		float halfWidth = width() * (0.5f * xFactor);
		float halfHeight = height() * (0.5f * yFactor);
	 ...
	 * inside, does nothing.
	 */
	public void enlargeToContain(float[] v) {
		enlargeToContain(v[0], v[1]);
	}
	...
	/**
	 * Moves the rect to be placed inbetween [bottom;top]. If 
	 * <pre>top - bottom < width()</pre>, the behavior is undefined.
	 * 
	 */
	public void moveInside(float bottom, float top) {
		if (this.top > top) moveY(top - this.top);
		if (this.bottom < bottom) moveY(bottom - this.bottom);
	}
